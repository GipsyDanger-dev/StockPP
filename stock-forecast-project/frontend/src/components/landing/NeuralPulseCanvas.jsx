...
export default function NeuralPulseCanvas() {
  ...
  useEffect(() => {
    ...
    const pulseLight = new THREE.PointLight(O, 0, 2.5)
    ...
    function pickNextPath() {
      if (pulseLayer >= LAYERS.length - 1) return null
      ...
      const ti = pulseLayer + 1 === LAYERS.length - 1 ? 0 : Math.floor(Math.random() * toNodes.length)
      ...
      if (pulseActive) {
        ...
        if (pulseProgress >= 1) {
          ...
          if (pulseLayer >= LAYERS.length - 1) {
            nodeMeshes[LAYERS.length - 1][0].mat.color.setHex(O)
            pulseActive = false
            pulseSphere.visible = false
            pulseLight.intensity = 0
            pulseWaiting = true
  ...
  return <div ref={ref} style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'auto' }} />
}

...
import { 
  TrendingUp, TrendingDown, Download, Settings, 
  LayoutDashboard, BarChart3, PieChart, Lightbulb, FileText, Search, Menu, X 
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center bg-slate-50 font-bold text-slate-400">LOADING AI ENGINE...</div>;

  return (
    <div className="flex min-h-screen bg-[#FFFFFF] text-[#191C1E]">
      
      {/* --- SIDEBAR (Desktop) / MOBILE NAV --- */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#F7F9FB] border-r border-[#C6C6CD] transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform lg:translate-x-0 lg:static`}>
      ...
      <main className="flex-1 overflow-x-hidden">
        ...
        <header className="bg-[#F7F9FB] px-6 py-4 flex justify-between items-center border-b border-[#C6C6CD]">
          ...
          <div className="flex items-center gap-4">
             <div className="hidden md:block text-right">
                <p className="text-xs font-bold text-slate-400">PREMIUM ACCOUNT</p>
        ...
        <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-10">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              <h1 className="text-5xl lg:text-7xl font-bold mb-4 tracking-tight">Market Insights</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white border-2 border-[#C6C6CD] rounded-2xl p-8 shadow-sm">
              <div className="flex justify-between items-start mb-8">
                <div>
                  <p className="text-[#45464D] font-bold text-sm tracking-widest uppercase">Portfolio Alpha</p>
                  <h2 className="text-4xl font-bold mt-1">{formatCurrency(data.current_price * 1000)}</h2>
                </div>
            ...
            <div className="space-y-8">
              <div className="bg-[#0D1C2F] text-white rounded-2xl p-8 flex flex-col justify-between h-full shadow-xl">
                <div>
                  <p className="text-[#76859B] font-bold text-sm tracking-widest mb-6">TOP AI PREDICTION</p>
                  <div className="flex items-center gap-4 mb-8">
                    <div className="bg-white text-black px-4 py-2 rounded font-bold text-2xl">{data.ticker}</div>
                    <div>
                  ...
                  <div className="space-y-6">
                    <div>
                      <p className="text-[#76859B] text-sm mb-2">Conviction Score</p>
                    ...
                    <div>
                      <p className="text-[#76859B] text-sm mb-1">Target Horizon</p>
                      <p className="text-2xl font-bold">30 Days</p>
                    </div>
                  </div>
                </div>
                <button className="w-full bg-white text-black py-4 rounded-lg font-bold mt-8 hover:bg-slate-200 transition-colors">
                  View Analysis
                </button>
              </div>
            </div>
          ...
          <div className="bg-white border-2 border-[#C6C6CD] rounded-2xl overflow-hidden shadow-sm">
            <div className="bg-[#F2F4F6] p-6 border-b border-[#C6C6CD] flex justify-between items-center">
              <h3 className="font-bold tracking-widest text-[#45464D]">LATEST SIGNALS</h3>
              <Activity size={20} className="text-[#45464D]"/>
            </div>
            <div className="divide-y divide-[#C6C6CD]">
              <SignalRow ticker="TSLA" action="SELL" time="10:42 AM" color="text-red-600" bg="bg-red-50" border="border-red-200" dot="bg-red-600" />
              <SignalRow ticker="MSFT" action="BUY" time="09:15 AM" color="text-emerald-600" bg="bg-emerald-50" border="border-emerald-200" dot="bg-emerald-600" />
              <SignalRow ticker="AAPL" action="HOLD" time="Yesterday" color="text-slate-600" bg="bg-slate-100" border="border-slate-200" dot="bg-slate-600" />
            </div>
          </div>
...
const NavItem = ({ icon, label, active = false }) => (
  <div className={`flex items-center gap-4 px-4 py-3 rounded-lg cursor-pointer transition-colors ${active ? 'bg-[#131B2E] text-white' : 'text-[#45464D] hover:bg-slate-200'}`}>
    {icon}
...
const SignalRow = ({ ticker, action, time, color, bg, border, dot }) => (
  <div className="flex items-center justify-between p-6 hover:bg-slate-50 transition-colors">
    ...
    <div className="flex items-center gap-12">
      <div className={`${bg} ${color} ${border} border-2 px-6 py-1 rounded font-bold text-sm`}>
        {action}
      </div>

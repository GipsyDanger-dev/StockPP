...
import { Search, Activity, LayoutDashboard, BarChart3, PieChart, Lightbulb, FileText, ChevronLeft, ChevronRight, Loader, TrendingUp, TrendingDown } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useMarketSummary, useTickerSearch } from '../hooks/useApi';
...
const Market = () => {
  ...
  // Fetch market data from Supabase via endpoint
  ...
  // Search via Finnhub when user types
  useEffect(() => {
  ...
  // Filter stocks by search term and sector (for Supabase stocks)
  ...
  // Determine what to show: search results or stock list
  const showSearchResults = searchTerm.length >= 2;
  const displayStocks = showSearchResults ? searchResults : filteredStocks;
      ...
      <main className="flex-1 overflow-x-hidden">
        <header className="bg-[#F7F9FB] px-6 lg:px-12 py-10 border-b border-[#E0E3E5]">
          ...
          <div className="flex gap-4 mt-8 flex-wrap">
            {['All Sectors', 'Technology', 'Healthcare', 'Finance', 'Automotive'].map(sector => (
              <button
                key={sector}
